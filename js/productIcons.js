...
(function () {
  ...
  const ALL_LINEAGE_CLASSES = Object.values(LINEAGE_VARIANT_CLASS_MAP).reduce(
    function (acc, classes) {
  ...
  function applyLineageVariantClasses(badge, variant) {
    const targetVariant = LINEAGE_VARIANT_CLASS_MAP[variant]
      ? variant
      : "balanced";
    ...
    badges.forEach(function (badge) {
      ...
      if (iconEl) {
        if (meta.icon) {
          ...
          if (iconEl.classList.contains("hidden"))
  ...
  function setPriceType(scope) {
    const root = scope || document;
    // iterate cards (adjust selectors to your card root)
    root.querySelectorAll(".group, .flex").forEach((card) => {
      const type = normalizeType(findTypeForCard(card)); // robust source of truth
      const price = card.querySelector(".price-per-gram");
      if (!price) return;
      const show = type === "flower";
      price.classList.toggle("hidden", !show);
      price.classList.toggle("flex", show);
    });
  }
